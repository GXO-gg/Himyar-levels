...
class Ranks(commands.Cog):
    ...
    async def render_board(self, guild: discord.Guild, key: str, page: int,
                           limit: Optional[int] = None) -> discord.Embed:
        settings, bag = await self.context(guild.id)
        label, column = BOARDS.get(key, BOARDS["xp"])
        size = max(1, int(limit or PAGE_SIZE))
        offset = max(0, page) * size
        rows = await self.bot.db.leaderboard(guild.id, column, size, offset)
